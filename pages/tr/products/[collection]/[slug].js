...
export default function ProductPageTR({ product, collection, relatedProducts }) {
  ...
  const collectionLabelTr = collection === 'luxury' ? 'Lüks Koleksiyon' : 'Çağdaş'
  const collectionLabel = collection === 'luxury' ? 'Luxury Collection' : 'Contemporary'
  const pageTitle = `${product.name} – ${collectionLabelTr} | Pouf Furniture`
  const canonicalUrl = `${SITE_URL}/tr/products/${collection}/${product.slug}/`
  ...
  const jsonLd = {
    ...
    '@type': 'Product',
    name: product.name,
    description: product.metaDescTr || product.metaDesc,
    image: images,
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: collectionLabelTr, item: `${SITE_URL}/tr/collection/` },
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={product.metaDescTr || product.metaDesc} />
        <meta httpEquiv="content-language" content="tr" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <meta property="og:title" content={pageTitle} />
        <meta property="og:description" content={product.metaDescTr || product.metaDesc} />
        ...
        <link rel="alternate" hrefLang="en" href={`${SITE_URL}/products/${collection}/${product.slug}/`} />
        <link rel="alternate" hrefLang="tr" href={canonicalUrl} />
        <link rel="alternate" hrefLang="x-default" href={`${SITE_URL}/products/${collection}/${product.slug}/`} />
        <script
      ...
      <section style={{ background: '#0D2118', paddingTop: 100, paddingBottom: 0, minHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
        <div className="max-w-screen-xl mx-auto px-6 lg:px-12 w-full pt-8">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 28 }}>
            ...
            <Link href="/tr/collection" style={{ fontFamily: "'League Spartan',sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(254,254,254,0.35)', textDecoration: 'none' }}>Koleksiyonlar</Link>
            <i className="ti ti-chevron-right" style={{ fontSize: 10, color: 'rgba(254,254,254,0.2)' }} />
            <Link href={`/tr/collection#${collection}`} style={{ fontFamily: "'League Spartan',sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(254,254,254,0.35)', textDecoration: 'none' }}>{collectionLabelTr}</Link>
            <i className="ti ti-chevron-right" style={{ fontSize: 10, color: 'rgba(254,254,254,0.2)' }} />
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-start pb-16">
            ...
            <div>
              ...
              >
                <img
                  ...
                  alt={`${product.name} — fotoğraf ${activeImg + 1}`}
                ...
                >
                  <i className="ti ti-zoom-in" style={{ fontSize: 14 }} /> Yakınlaştır
                </button>
                ...
                <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4, scrollbarWidth: 'thin', scrollbarColor: '#FF914D transparent' }}>
                  {Array.from({ length: product.imageCount }, (_, i) => (
                    <div
                      ...
                      onClick={() => goImg(i)}
                    >
                      <img src={imgUrl(collection, product.slug, i + 1)} alt={`Fotoğraf ${i + 1}`} loading="lazy" />
                    </div>
                  ))}
                </div>
              )}
            ...
            <div>
              ...
              <div className="prod-category-tag" style={{ marginBottom: 10 }}>
                <i className={`ti ${product.categoryIcon}`} style={{ fontSize: 10 }} /> {product.categoryTr || product.category}
              </div>
              ...
              <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 17, lineHeight: 1.75, color: 'rgba(254,254,254,0.65)', marginBottom: 32 }}>
                {product.descTr || product.desc}
              </p>

              {/* Spec tags */}
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 36 }}>
                {[
                  { icon: 'ti-map-pin', label: 'İnegöl, Türkiye\'de Üretildi' },
                  { icon: 'ti-ship', label: 'İhracata Hazır' },
                  { icon: 'ti-adjustments-horizontal', label: 'OEM Mevcut' },
                  { icon: 'ti-shield-check', label: '2 Yıl Garanti' },
                ].map(tag => (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <Link href="/tr/#contact" className="btn btn-orange" style={{ padding: '16px 32px', justifyContent: 'center' }}>
                  Ticari Teklif İste <i className="ti ti-arrow-right" />
                </Link>
                ...
                <Link href={`/tr/collection#${collection}`} className="btn-ghost" style={{ justifyContent: 'center' }}>
                  <i className="ti ti-arrow-left" /> {collectionLabelTr}'e Geri Dön
                </Link>
              </div>
            </div>
          </div>
      ...
      {relatedProducts.length > 0 && (
        <section style={{ background: '#1C3A2E', padding: '80px 0' }}>
          <div className="max-w-screen-xl mx-auto px-6 lg:px-12">
            <div className="eyebrow reveal" style={{ color: '#FF914D', marginBottom: 14 }}>{collectionLabelTr}'den Daha Fazlası</div>
            <h2 className="sec-heading reveal reveal-d1" style={{ fontSize: 'clamp(28px,4vw,48px)', color: '#FEFEFE', marginBottom: 40 }}>Benzer Modeller</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {relatedProducts.map((p, i) => (
                <Link
                  key={p.slug}
                  href={`/tr/products/${collection}/${p.slug}`}
                ...
                >
                  ...
                  <div className="prod-body">
                    <div className="prod-category-tag"><i className={`ti ${p.categoryIcon}`} style={{ fontSize: 9 }} /> {p.categoryTr || p.category}</div>
                    <div className="prod-name">{p.name}</div>
                    <div className="prod-desc" style={{ WebkitLineClamp: 2, overflow: 'hidden', display: '-webkit-box', WebkitBoxOrient: 'vertical' }}>{p.descTr || p.desc}</div>
                    <div className="prod-actions">
                      <div className="chip chip-white" style={{ fontSize: 9, padding: '4px 10px' }}>İhracata Hazır</div>
                      <span className="enquire-btn">Modeli Gör <i className="ti ti-arrow-right" style={{ fontSize: 10 }} /></span>
                    </div>
                  </div>
                </Link>
              ))}
      ...
      {lbOpen && (
        ...
        >
          <div id="lb-header">
            <div>
              <div className="eyebrow" style={{ color: '#FF914D', marginBottom: 4 }}>{collectionLabelTr}</div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <button id="lb-close" onClick={() => setLbOpen(false)} aria-label="Kapat"><i className="ti ti-x" /></button>
            </div>
          </div>
          <div id="lb-main-wrap" onTouchStart={lbTouchStart} onTouchEnd={lbTouchEnd}>
            {product.imageCount > 1 && (
              <button className="lb-arrow" style={{ left: 14 }} onClick={() => goImg((activeImg - 1 + product.imageCount) % product.imageCount)} aria-label="Önceki">
                <i className="ti ti-chevron-left" />
              </button>
            )}
            <img
            ...
            />
            {product.imageCount > 1 && (
              <button className="lb-arrow" style={{ right: 14 }} onClick={() => goImg((activeImg + 1) % product.imageCount)} aria-label="Sonraki">
                <i className="ti ti-chevron-right" />
              </button>
            )}
          </div>
